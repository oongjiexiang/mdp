...
void update_h_cost(Vertex* vertex_array, Vertex* src_vertex, Vertex* target_vertex, int total_grid_count);//caculates the hcost, gcost+heuristic value, distance from source and estimated distance from target, distance from source
void print_path(Vertex* src_vertex, Vertex* target_vertex);

//----not done----
Vertex* a_star_search(Vertex* vertex_array, Vertex* adj_matrix[total_grid_count][total_grid_count], Vertex* src_vertex, Vertex* target_vertex, int total_grid_count);
double* trip_planning(Vertex* obstacle_array, Vertex* vertex_array, Robot* robot);//a*search with obstacle avoidance, returns the path and direction(theta)
bool obstacle_avoidance(Vertex* obstacle_array, Robot* robot);//obstacle avoidance checking and redirection, returns true if obstacle found, false if no obstacle
...
//----vertex functions----
void initialize_vertex(Vertex* vertex_array,int row_count, int column_count,int unit_length,int total_count);//initialize all the vertex variables
void reset_vertex(Vertex* vertex_array,int row_count, int column_count,int unit_length,int total_count);//reset the cost, obstacle, border, visited, and prev_vertex for all vertices, use this every time after 1 path has been found
void generate_adj_matrix(Vertex* adj_matrix[total_grid_count][total_grid_count],Vertex* vertex_array, int row_count, int column_count, int total_count);
Vertex* find_vertex(Vertex* vertex_array, double x_coor, double y_coor, int total_count);//assumes the x and y coordinates given refer to the middle of the object
void add_obstacle(Vertex* vertex_array, Obstacle* obstacle_array, int no_of_obstacles, double obstacle_length,double boundary_size, int total_count);//add obstacles and borders
...
int main()
{
    ...
    Vertex* vertex_array= (Vertex*)malloc(total_grid_count*sizeof(Vertex));
    ...
    Vertex* adj_matrix[total_grid_count][total_grid_count];
    Obstacle obstacle_array[1] = {{0,50 ,10}};
...
Vertex* a_star_search(Vertex* vertex_array, Vertex* adj_matrix[total_grid_count][total_grid_count], Vertex* src_vertex, Vertex* target_vertex, int total_grid_count){
    ...
    Sorted_Queue* q = (Sorted_Queue*)malloc(sizeof(Sorted_Queue));
...
void update_h_cost(Vertex* vertex_array, Vertex* src_vertex, Vertex* target_vertex, int total_grid_count){
    int i, x1, x2, y1, y2;
    src_vertex->h_cost=0;
...
void initialize_vertex(Vertex* vertex_array,int row_count, int column_count,int unit_length,int total_count){
    ...
    for(i=0;i<row_count;i++){
        for(j=0;j<column_count;j++){
            ...
            vertex_array[current_index].x_right =((j+1)*unit_length)-1;  // JX: why -1?
            vertex_array[current_index].x_left =j*unit_length;
            vertex_array[current_index].y_high =((i+1)*unit_length)-1;   // JX: why -1?
...
void reset_vertex(Vertex* vertex_array,int row_count, int column_count,int unit_length,int total_count){
    int i,j,start_cost,current_index;
    start_cost = INT_MAX;
    for(i=0;i<row_count;i++){
        for(j=0;j<column_count;j++){
...
void add_obstacle(Vertex* vertex_array, Obstacle* obstacle_array, int no_of_obstacles, double obstacle_length,double boundary_size, int total_count){
    int i,j;
...
Vertex* find_vertex(Vertex* vertex_array, double x_coor, double y_coor, int total_count){
    int i;
    Vertex* result=NULL;
    /*
    ...
    */
    ...
    i = ceil(y_coor/ unit_length)*column_count + ceil(x_coor/unit_length);
    if(i >= 0 && i < total_count) result = &vertex_array[i];
    if(result==NULL){
